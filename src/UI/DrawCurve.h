...
namespace UI
{
    class DrawCurve : public QObject
    {
        ...
        Q_PROPERTY(double maxHistHP
                   READ getMaxHistHP)
        Q_PROPERTY(double minHistHL
                   READ getMinHistHL)
        Q_PROPERTY(double maxHistHL
                   READ getMaxHistHL)
        Q_PROPERTY(QVector<double> curTableTemp
                   READ getCurTableTemp
                   NOTIFY sendCurTableTemp)
        Q_PROPERTY(QVector<int> leakageDetection
                   READ getLeakageDetection
                   NOTIFY sendLeakageDetection)
    ...
    public:
        ...
        QVector<QVector<QPointF>> getHistoryHeatPower();
        QVector<QVector<QPointF>> getHistoryEnvTemp();
        QVector<QVector<QPointF>> getHistoryTemp();

        Q_INVOKABLE QString loadSwitchText();
        Q_INVOKABLE QString loadHistSwitchText();
        Q_INVOKABLE QList<int> getHistoryCurvedCount();
        Q_INVOKABLE QVector<QString> getCurvedColor();
    ...
    signals:
        ...
        void sendLeakageDetection();
        void sendHistHeatLoss();
    ...
    public slots:
        ...
        void updateHistoryHeatLossGraph(QAbstractSeries *series, const int index);

    private:
        QVector<QString> m_curveColor={
            "dimgray","saddlebrown","darkgreen","olive","darkslateblue","darkcyan","yellowgreen","darkblue","darkseagreen","darkmagenta",
            "maroon3","orangered","orange","orange","chartreuse","blueviolet","crimson","aqua","deepskyblue","blue",
            "lightsteelblue","coral","fuchsia","dodgerblue","lightgreen","deeppink","violet","moccasin","lightpink"};
        const int m_displayCount = 600;
        ...
        double m_minHistHL, m_maxHistHL;
        ...
        QVector<QVector<QPointF>> m_historyHeatLoss;
        ...
        QVector<int> m_leakageDetection;
    ...
    public:
        ...
        QVector<int> getLeakageDetection()
        {
            return m_leakageDetection;
        }

        //定积分
        double integral(qreal value, long n);
        ...
        QVector<QVector<QPointF>> getHeatLoss(QVector<QVector<QPointF>> heatPower);
